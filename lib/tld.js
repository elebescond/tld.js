 ...
 *
 * @todo the parsing logic should not located here for many reasons (format can change etc.)
...
tld.prototype.getRulesForTld = function getRulesForTld (tld, default_rule) {
  ...
  // Parsing needed
  this.rules[tld] = [];
  rules.split(' ').forEach(function transformAsRule (rule, i) {
    var typeFlag = rule[0];
    var xld = rule.slice(1);

    self.rules[tld].unshift(new Rule({
      "firstLevel":  tld,
      "secondLevel": i === 0 ? null : xld,
      "exception":   !!(typeFlag & Rule.TYPE_EXCEPTION),
      "wildcard":    !!(typeFlag & Rule.TYPE_WILDCARD)
    }));
  });

  return this.rules[tld];
};

tld.prototype.getRulesForTldJSON = function getRulesForTld (tld, default_rule) {
  var exception = '!';
  var wildcard = '*';
...
/**
 * Bootstraps rules based on the optimised text storage
 *
 * @param {String} textRules
 */
tld.prototype.updateFromText = function updateFromText(textRules){
  var self = this;
  this.rules = {};

  textRules.split("\n").forEach(function(row){
    var spacePos = row.indexOf(' ');

    if (spacePos === -1){
      spacePos = undefined;
    }

    var tld = row.slice(1, spacePos);
